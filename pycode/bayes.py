...
class Bayes:
    ...
    #Converts a sentence to a list of features. If a word isn't in the features, it looks if its synonims are in the list of features.
    def document_features_sin(self,sent):
        features = {}
        common = list(self.features & set(sent))
        sent_temp = list(sent)
        for word in sent:
            if word not in common:
                sent_temp += self.returnSynonims(word)
        sent_temp.extend(nltk.bigrams(sent))
        common = list(self.features & set(sent_temp))
        for feat in common:
            features[feat] = True
        return features
    ...
    def returnSynonims(self,word):
        try:
            return wn.synsets(word)[0].lemma_names
        except:
            return []

    #returns our grade computed for a sentence using synonims
    def grade_sentence_sin(self,sent):
        test = self.document_features_sin(sent)
